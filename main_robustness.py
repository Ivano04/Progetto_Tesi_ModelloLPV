...
def run_robustness_simulation(scenario_key, usa_rk4=True, live_plot=True):
    ...
    if live_plot:
        ...
        # Tracciato target in background
        ...
        ax.set_title(f"Robustness Live: {track_name}\n({integrator_type} - {supervisor_status})", fontsize=10)
        ax.grid(True, alpha=0.2)


    print(f"AVVIO ROBUSTEZZA: {track_name} | Rumore: {noise_magnitude}")
    ...
    for i in range(steps):
        ...
        if live_plot and i % 250 == 0:
            ...
            # Camera Follow centrata sul veicolo (Zoom 8 metri)
    ...
    save_metadata(run_dir,
                  {"Test": "Robustezza", "Noise": noise_magnitude, "Int": integrator_type},
                  {"RMSE": f"{rmse:.5f}m"})
...
if __name__ == "__main__":
    ...
    # SELEZIONE CIRCUITO: 'racing', 'eight', 'circular'
    circuito_selezionato = 'circular'


    # Esecuzione singola ottimizzata
